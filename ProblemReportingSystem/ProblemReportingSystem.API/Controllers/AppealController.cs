...
public class AppealController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateAppeal([FromForm] CreateAppealRequest request,
        CancellationToken cancellationToken = default)
    {
        ...
        var createAppealDto = _mapper.Map<AppealDto>(request);

        // Ініціалізуємо колекцію фотографій, якщо вона ще не створена
        createAppealDto.ProblemDto.Photos = new List<CreateProblemPhotoDto>();
        ...
        if (request.Photos != null && request.Photos.Any())
        {
            foreach (var file in request.Photos)
            {
                if (file.Length > 0)
                {
                    ...
                    createAppealDto.ProblemDto.Photos.Add(new CreateProblemPhotoDto
                    {
        ...
        var problemId = await _appealService.CreateAppealAsync(createAppealDto);

        _logger.LogInformation($"Problem created successfully with ID: {problemId}");
        return CreatedAtAction(nameof(CreateAppeal), new { id = problemId }, new { id = problemId });
    }
}
...
public async Task<IActionResult> GetProblemById(Guid id)
{
    ...
    var problem = await _appealService.GetProblemByIdAsync(id);
...
public async Task<IActionResult> GetAllProblems()
{
    ...
    var problems = await _appealService.GetAllProblems();
...
public async Task<IActionResult> UpdateProblem([FromBody] UpdateProblemRequest request)
{
    ...
    var problemId = await _appealService.UpdateProblem(updateProblemDto);
...
public async Task<IActionResult> GetUserProblems(Guid userId)
{
    ...
    var problems = await _appealService.GetUserProblems(userId);
    var summaryResponses = _mapper.Map<IEnumerable<SummaryProblemResponse>>(problems);
...
public async Task<IActionResult> GetProblemsByStatus(string status)
{
    ...
    var problems = await _appealService.GetProblemsByStatus(status);
    var summaryResponses = _mapper.Map<IEnumerable<SummaryProblemResponse>>(problems);
...
public async Task<IActionResult> GetProblemsByCategory(Guid categoryId)
{
    ...
    var problems = await _appealService.GetProblemsByCategory(categoryId);
    var summaryResponses = _mapper.Map<IEnumerable<SummaryProblemResponse>>(problems);
...
public async Task<IActionResult> GetProblemsInBounds([FromBody] GetProblemsInBoundsRequest request)
{
    ...
    var problems = await _appealService.GetProblemsInBounds(request.MinLatitude, request.MaxLatitude, request.MinLongitude, request.MaxLongitude);
    var summaryResponses = _mapper.Map<IEnumerable<SummaryProblemResponse>>(problems);
...
public async Task<IActionResult> GetAssignedProblems(Guid employeeId)
{
    ...
    var problems = await _appealService.GetAssignedProblems(employeeId);
    var summaryResponses = _mapper.Map<IEnumerable<SummaryProblemResponse>>(problems);
